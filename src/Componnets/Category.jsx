        ...
        {data?.map((item) => (
          <div className="p-2 border border-[#b65a18] rounded-lg" key={item._id}>
           ...
           <h2>
            ...
            <h4 className="mb-5"><span>Rating :</span>{item.rate}</h4>
           </div>
            <div className="my-4">
            <Link onClick={ () => handdleCategory(item._id)} className="py-2 px-5 bg-[#b65a18] rounded-lg text-white mt-5" to={`/categorydetails/${item._id}`}>Details</Link>
            </div>
          </div>
        ))}
      </div>
